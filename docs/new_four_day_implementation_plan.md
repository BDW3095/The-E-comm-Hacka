...
RankingConfig:
  color_conflict_mode: str     # conservative；Q6 确认前不得设 hard_constraint
  reliable_color_penalty: float # 明确且单一 non-match color 的小幅扣分
~~~
...
- color_conflict_mode 默认 conservative：可靠的正向 color 加分，可靠且单一的相反 color 只小幅扣分，缺失、多个或仅 description 中出现的颜色一律中性。只有主办方确认 parent_asin 为可靠 single variant，且独立实验得分提升，才能试验 hard_constraint。
...
- 实现 ConstraintRanker.rerank(candidates, state)。category、material、color、brand、size/style、feature/use_case 匹配加分；negative conflict 大幅扣分；已知 price 超预算小幅扣分；price=None 不扣分也不删除。color 必须使用三值 Conflict Scoring：可靠 black 加分；可靠、单一 non-black 仅小幅扣分；缺失、多色、不可靠或仅 description 命中的 color 保持中性。Q6 未明确前不得 hard filter。
...
- Boundary 后 other 重试，other exhausted，Turn 10 None。
...
- 默认 config：mode="simulator_optimized"、final_turn=10、SEMANTIC_ENABLED=False、color_conflict_mode="conservative"。D 将主办方 Q&A 的答复集中记录并映射为 config，不在 src.agent.py 中散落 special case。
...
| Owner | 当天工作 | 当天预期结果 |
|---|---|---|
| A | Top-200 lexical retrieval、constraint rerank、Top-10 validation | query、negative、null price、稳定排序、保守 color conflict 均有 test |
...
| D/PM | 向主办方发送 docs/organizer_engineer_qa.md 的 Q1–Q6，并记录答复 | 对 Q1、Q4、Q5、Q6 形成可执行的 config / scope 决定；未回复不阻塞 E2 |
...
- 不要在 parent_asin 的 variant 语义未确认前，用相反 color 将商品 hard filter；保持三值 Conflict Scoring。
